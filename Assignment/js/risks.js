...
function buildRegisterSuggestion(cveId, cvssValue, epssScore) {
  ...
  explanation.textContent = `${likelihoodSource}; ${impactSource}. Score ${likelihood} x ${impact} = ${likelihood * impact} → ${band.label.toLowerCase()} band.`;
...
function renderHeatMap(risks) {
  ...
  legend.append("(score = likelihood x impact: 15+ high, 8-12 medium, ≤6 low)");
  container.append(legend);
}

// Utility function that creates and configures SVG elements,
// promoting consistency and reducing repetitive DOM code.
...
// Equivalent data is provided through the accessible heat-map table,
// allowing the SVG visualisation to be treated as decorative content
// for assistive technologies.
  ...
  // number of risks so a 2-risk bubble does not look 4x bigger.
...
function renderLookupResult(cveId, cve, epssRecord, notes = []) {
  ...
  // Implements graceful degradation by displaying available risk data
  // even when a related source response is unavailable, preserving
  // functionality during partial service failures.
  ...
  // Validates numeric values before rendering, preventing incomplete
  // or non-numeric data from producing invalid user-facing output.
...
// Maps vulnerability intelligence to register scoring criteria by
// using EPSS as a likelihood indicator and CVSS as an impact measure,
// supporting traceable risk assessment and risk-register creation.
...
async function fetchJSON(url, providerName) {
  ...
  if (!response.ok) {
  // Includes provider and status information so callers can present
  // context-specific guidance and handle service-specific conditions
  // without relying on a generic error response.
...
/* Per-provider session cache that reduces duplicate API requests and
   improves resilience to rate limiting. Cached responses minimise
   unnecessary lookups and allow unaffected providers to continue
   supplying data during partial service failures.
*/
...
/* Version 23: Provides recent CVE suggestions from an external data source 
    while preserving full manual entry functionality. This supports graceful
   degradation and ensures the feature remains usable during service
   outages or rate-limiting conditions.
*/
...
lookupForm.addEventListener("submit", async event => {
  ...
  try {
    // Uses Promise.allSettled() so independent provider failures do not
    // prevent successful responses from other services being processed.
